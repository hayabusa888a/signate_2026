...
print(f"\n=== v2 OOF best F1={best_f1:.4f} at threshold={best_th:.2f} ===")
print("(baseline 0.6379 / feature_eng 0.6635 / tfidf 0.7168)")

# エラー分析用に OOF 予測を保存
np.save("../features/oof_pred_v2.npy", oof_pred)
pd.DataFrame({"企業ID": train[id_col], "oof_pred": oof_pred, "y": y,
              "threshold": best_th}).to_csv("../features/oof_pred_v2.csv", index=False)
print("saved features/oof_pred_v2.csv (for error analysis)")

# 重要度（最終fold）
imp = pd.Series(model.feature_importance(importance_type="gain"),
                index=X_tr_f.columns).sort_values(ascending=False)
imp.to_csv("../features/importance_v2.csv", encoding="utf-8-sig")
print("\nTop 25 importances:")
