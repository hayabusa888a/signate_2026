...
E5_COLS = ["今後のDX展望", "企業概要", "組織図"]
E5_PCA = {"今後のDX展望": 50, "企業概要": 30, "組織図": 20}
...
# BERT埋め込みロード
e5_tr = {c: np.load(f"../features/e5_{c}_train.npy") for c in E5_COLS}
e5_te = {c: np.load(f"../features/e5_{c}_test.npy") for c in E5_COLS}
...
def run(mode):  # mode: "none" | "add" | "replace"
    use_bert = mode in ("add", "replace")
    ...
    for tr_idx, va_idx in skf.split(Xs, y):
        ...
        # BERT: fold内でPCA圧縮
        e5_tr_parts, bert_va_parts = [], []
        if use_bert:
            for c in E5_COLS:
                Etr = e5_tr[c][tr_idx]; Eva = e5_tr[c][va_idx]
                bsc = StandardScaler().fit(Etr)
                pca = PCA(n_components=E5_PCA[c], random_state=RANDOM_STATE)
                pt = pca.fit_transform(bsc.transform(Etr)); pv = pca.transform(bsc.transform(Eva))
                ps = StandardScaler().fit(pt)
                e5_tr_parts.append(ps.transform(pt)); bert_va_parts.append(ps.transform(pv))
        ...
        extra_tr = [sparse.csr_matrix(np.hstack(e5_tr_parts))] if use_bert else []
        extra_va = [sparse.csr_matrix(np.hstack(bert_va_parts))] if use_bert else []
        # replace: TF-IDF SVD を使わず e5 のみ / add: 両方 / none: TF-IDFのみ
        svd_part_tr = [] if mode == "replace" else [svd_blk_tr]
        svd_part_va = [] if mode == "replace" else [svd_blk_va]
        raw_part_tr = [] if mode == "replace" else raw_tr
        raw_part_va = [] if mode == "replace" else raw_va

        Xsvd_tr = sparse.hstack(base_tr + svd_part_tr + extra_tr).tocsr()
        Xsvd_va = sparse.hstack(base_va + svd_part_va + extra_va).tocsr()
        Xraw_tr = sparse.hstack(base_tr + raw_part_tr + extra_tr).tocsr()
        Xraw_va = sparse.hstack(base_va + raw_part_va + extra_va).tocsr()
...
for mode, label in [("none", "現行(TF-IDFのみ)"), ("add", "TF-IDF+e5"), ("replace", "e5のみ(TF-IDF置換)")]:
    f, s = run(mode)
    print(f"[{label}] アンサンブル OOF F1={f:.4f}  個別={ {k: round(v,4) for k,v in s.items()} }", flush=True)
