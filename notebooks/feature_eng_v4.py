...
# v4b: word次元はv3と同じ40均一に戻し、char n-gramだけ追加して効果を切り分け
SVD_CONFIG = {
    "企業概要":     {"word": 40, "char": 20},
    "今後のDX展望": {"word": 40, "char": 20},
    "組織図":       {"word": 40, "char": 20},
}
...
# v4b: word系はv3と同じ full 分かち書き（品詞フィルタ無し）に戻す
tokenizer = Tokenizer()
...
print("tokenizing (full wakati)...", flush=True)
# word用: 品詞フィルタ済みトークン / char用: 生テキスト
train_tok = {c: train[c].fillna("").map(tok_content) for c in TFIDF_TEXT_COLS}
test_tok = {c: test[c].fillna("").map(tok_content) for c in TFIDF_TEXT_COLS}
train_raw = {c: train[c].fillna("").tolist() for c in TFIDF_TEXT_COLS}
test_raw = {c: test[c].fillna("").tolist() for c in TFIDF_TEXT_COLS}
...
def _svd_block(vec, n_svd, tag, col, tr_texts, va_texts, test_texts, seed):
    ...
    cols = [f"{col}_{tag}{i}" for i in range(n_comp)]
    return (pd.DataFrame(tr_svd, columns=cols),
            pd.DataFrame(va_svd, columns=cols),
            pd.DataFrame(test_svd, columns=cols))


def build_text_features(col, tr_idx, va_idx, seed):
    """word(品詞フィルタ) + char(生テキスト) の2系統TF-IDF+SVDを結合。"""
    cfg = SVD_CONFIG[col]
    parts_tr, parts_va, parts_te = [], [], []

    # word系: 品詞フィルタ済み, 1-2gram
    wvec = TfidfVectorizer(min_df=3, max_df=0.9, ngram_range=(1, 2), max_features=20000)
    w = _svd_block(wvec, cfg["word"], "wsvd", col,
                   [train_tok[col].iloc[i] for i in tr_idx],
                   [train_tok[col].iloc[i] for i in va_idx],
                   test_tok[col].tolist(), seed)
    # char系: 生テキスト, char_wb 2-4gram
    cvec = TfidfVectorizer(analyzer="char_wb", ngram_range=(2, 4), min_df=5, max_features=30000)
    c = _svd_block(cvec, cfg["char"], "csvd", col,
                   [train_raw[col][i] for i in tr_idx],
                   [train_raw[col][i] for i in va_idx],
                   test_raw[col], seed)
    for blk in (w, c):
        if blk[0] is not None:
            parts_tr.append(blk[0]); parts_va.append(blk[1]); parts_te.append(blk[2])
    return (pd.concat(parts_tr, axis=1) if parts_tr else None,
            pd.concat(parts_va, axis=1) if parts_va else None)
...
for fold, (tr_idx, va_idx) in enumerate(skf.split(X, y)):
    ...
    # TF-IDF(word+char) + SVD（fold内fit）
    tr_parts, va_parts = [X_tr], [X_va]
    for c in TFIDF_TEXT_COLS:
        tr_s, va_s = build_text_features(c, tr_idx, va_idx, RANDOM_STATE + fold)
...
print(f"\n=== v4 OOF best F1={best_f1:.4f} at threshold={best_th:.2f} ===")
print("(baseline 0.6379 / feature_eng 0.6635 / tfidf 0.7168 / v2 0.7442 / v3 0.7471)")

# エラー分析用に OOF 予測を保存
np.save("../features/oof_pred_v4.npy", oof_pred)
pd.DataFrame({"企業ID": train[id_col], "oof_pred": oof_pred, "y": y,
              "threshold": best_th}).to_csv("../features/oof_pred_v4.csv", index=False)
print("saved features/oof_pred_v4.csv (for error analysis)")

# 重要度（最終fold）
imp = pd.Series(model.feature_importance(importance_type="gain"),
                index=X_tr_f.columns).sort_values(ascending=False)
imp.to_csv("../features/importance_v4.csv", encoding="utf-8-sig")
print("\nTop 30 importances:")
print(imp.head(30).to_string())
# テキスト系(word/char)の寄与を集計
word_gain = imp[[i for i in imp.index if "_wsvd" in str(i)]].sum()
char_gain = imp[[i for i in imp.index if "_csvd" in str(i)]].sum()
print(f"\n[テキスト寄与合計] word系SVD gain={word_gain:.0f} / char系SVD gain={char_gain:.0f}")
for col in TFIDF_TEXT_COLS:
    g = imp[[i for i in imp.index if str(i).startswith(col + "_")]].sum()
    print(f"  {col}: gain合計={g:.0f}")
